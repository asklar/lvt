...
TEST_F(WinUI3SampleFixture, WatchControlGetsSetsAndClearsNativeProperty) {
    SkipIfNotReady();

    auto lvt = get_lvt_path();
    auto winui3Ready = [](const json& j) {
        return frameworks_contain_winui3(j) && j.contains("root") &&
               json_tree_has_named_control(j["root"], "PrimaryButton");
    };
    auto tree = dump_ready_tree(lvt, get_pid_arg() + " --fast", winui3Ready);
    ASSERT_TRUE(winui3Ready(tree));
    auto* button = find_named_control(tree["root"], "PrimaryButton");
    ASSERT_NE(button, nullptr);
    const std::string key = button->value("key", "");
    ...
    ASSERT_TRUE(CreatePipe(stdoutRead.put(), stdoutWrite.put(), &sa, 0));
    ASSERT_TRUE(CreatePipe(stdinRead.put(), stdinWrite.put(), &sa, 0));
    ASSERT_TRUE(SetHandleInformation(stdoutRead.get(), HANDLE_FLAG_INHERIT, 0));
    ASSERT_TRUE(SetHandleInformation(stdinWrite.get(), HANDLE_FLAG_INHERIT, 0));
    ...
    std::string cmd = make_cmd(
        lvt, get_pid_arg() + " watch --fast --control --interval 200");
    ASSERT_TRUE(CreateProcessA(nullptr, cmd.data(), nullptr, nullptr, TRUE,
                               CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi));
    wil::unique_handle process(pi.hProcess);
    wil::unique_handle thread(pi.hThread);
    ...
    auto send_request = [&](const json& request) {
        std::string line = request.dump() + "\n";
        DWORD written = 0;
        ASSERT_TRUE(WriteFile(stdinWrite.get(), line.data(),
                              static_cast<DWORD>(line.size()), &written, nullptr));
        ASSERT_EQ(written, line.size());
    };

    std::string bufferedOutput;
    auto read_result = [&](uint64_t requestId) {
        const auto deadline = GetTickCount64() + 60000;
        while (GetTickCount64() < deadline) {
            size_t newline = 0;
            while ((newline = bufferedOutput.find('\n')) != std::string::npos) {
                std::string line = bufferedOutput.substr(0, newline);
                bufferedOutput.erase(0, newline + 1);
                if (!line.empty() && line.back() == '\r')
                    line.pop_back();
                auto event = json::parse(line, nullptr, false);
                if (!event.is_discarded() &&
                    event.value("event", "") == "command_result" &&
                    event.value("requestId", uint64_t{0}) == requestId) {
                    return event;
                }
            }

            DWORD available = 0;
            if (!PeekNamedPipe(stdoutRead.get(), nullptr, 0, nullptr, &available, nullptr))
                break;
            if (available == 0) {
            ...
            std::string chunk(available, '\0');
            DWORD read = 0;
            if (!ReadFile(stdoutRead.get(), chunk.data(), available, &read, nullptr) || read == 0)
                break;
            bufferedOutput.append(chunk, 0, read);
        }
        return json();
    };
    ...
    auto propertiesResult = read_result(1);
    ASSERT_FALSE(propertiesResult.is_null()) << "get_properties timed out";
    ASSERT_TRUE(propertiesResult.value("ok", false))
        << propertiesResult.value("error", "");
    ASSERT_TRUE(propertiesResult.contains("properties"));

    json opacity;
    for (const auto& property : propertiesResult["properties"]) {
    ...
    ASSERT_EQ(opacity.value("metadataBits", uint64_t{0}) & 0x2, 0u)
        << "Opacity unexpectedly reported read-only";
    ...
    auto setResult = read_result(2);
    ASSERT_FALSE(setResult.is_null()) << "set_property timed out";
    ASSERT_TRUE(setResult.value("ok", false)) << setResult.value("error", "");
    ...
    auto changed = read_result(3);
    ASSERT_TRUE(changed.value("ok", false)) << changed.value("error", "");
    auto opacity_value = [](const json& result) -> std::string {
    ...
    ASSERT_FALSE(opacity_value(changed).empty());
    EXPECT_NEAR(std::stod(opacity_value(changed)), 0.5, 0.001);

    send_request({
        {"requestId", 4},
        {"command", "clear_property"},
        {"key", key},
        {"propertyIndex", propertyIndex},
    });
    auto clearResult = read_result(4);
    ASSERT_TRUE(clearResult.value("ok", false)) << clearResult.value("error", "");
    ...
    auto cleared = read_result(5);
    ASSERT_TRUE(cleared.value("ok", false)) << cleared.value("error", "");
    ASSERT_FALSE(opacity_value(cleared).empty());
    EXPECT_NEAR(std::stod(opacity_value(cleared)), 1.0, 0.001);

    // Let multiple normal GET_TREE ticks run on the same connection after all
    // three command kinds, then verify that none caused another SetSite.
    Sleep(700);
    TerminateProcess(process.get(), 0);
    WaitForSingleObject(process.get(), 5000);
    EXPECT_EQ(count_set_site_calls() - before, 1)
        << "watch --control must use one persistent TAP injection for tree ticks "
           "and get/set/clear commands";
}
