// WinForms_tap_native.cpp — Native DLL injected into WinForms target process.
// Hosts the .NET CLR and calls managed WinFormsTreeWalker.CollectTree().
...
static void LogMsg(const char* fmt, ...) {
    ...
    if (!logFile) {
        ...
        wcscat_s(tmp, L"lvt_WinForms_tap.log");
...
static bool TryNetCore(const std::wstring& assemblyPath, const std::wstring& pipeName) {
    ...
    // Get function pointer to WinFormsTreeWalker.CollectTree
    ...
    rc = loadAssembly(
        ...
        L"CollectTree",
        L"LvtWinFormsTap.WinFormsTreeWalker+CollectTreeDelegate, LvtWinFormsTap",
        nullptr,
...
static DWORD WINAPI WorkerThread(LPVOID lpParameter) {
    ...
    std::wstring dir = GetDllDirectory();
    std::wstring assemblyPath = dir + L"\\LvtWinFormsTap.dll";
